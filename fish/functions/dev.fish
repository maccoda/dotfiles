function dev
    ...
    if test -z $command
        ...
        gh search prs --author=@me --state=open --owner=$github_org --draft=false --review=required --json number,title,url,author,createdAt --template $template -- NOT "[Snyk]"
        heading --no-trail "Approved created reviews"
        gh search prs --author=@me --state=open --owner=$github_org --draft=false --review=approved --json number,title,url,author,createdAt --template $template -- NOT "[Snyk]"
